...
\cs_new:Npn \generatelistofrequisitestableline {
  \seq_map_inline:Nn \g_luke_listofrequisites_seq  {
    ...
    \seq_if_empty:NF \l_tmpa_seq {
    \use:c{##1h} & \use:c{##1r} & \seq_set_from_clist:Nx \l_tmpa_seq {\getrefnumber{requisite::##1}}
...
% First Argument:  The Requisite's full name (as shown in the document)
...
\NewDocumentCommand{\NewRequisite}{m+m+m+m+m}{%
  ...
  \expandafter\NewDocumentCommand\csname #2r\endcsname{}{%
    #5%
  } % defines \<charactername>_role-command to print the requisite who plays that character.
}% End of \NewRequisite
\makeatother
...
% First Argument:  The Requisite's full name (as shown in the document)
% Second Argument: The Requisite's short name (used for the command creation)
% Third Argument: The requisite who plays that character (the role).
\makeatletter
\NewDocumentCommand{\NewRequisiteAutoColor}{m+m+m}{%
  \typeout{Creating Requisite #1 with automatic colors...}
