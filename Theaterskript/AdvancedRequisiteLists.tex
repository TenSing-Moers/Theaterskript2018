...
\NewDocumentCommand{\DisplayRequisites}{}{%
  \seq_clear:N \l_luke_listofrequisites_seq % Clear l_luke_listofrequisites_seq
  \group_begin:
  \seq_clear:N \l_tmpa_seq % Clear temp list
  \seq_map_inline:Nn \g_luke_listofpersons_seq {%
   ...
   \seq_if_empty:NF \l_tmpa_seq {
     \PrePersonList
     \seq_use:Nn \l_luke_listofrequisites_seq {,~} % this is the delimiter
     \PostPersonList
   %}
   \group_end:
}
...
\makeatletter
\NewDocumentCommand{\requisite}{m+m}{%
...
%  \IfRequisiteCalledAlreadyF{#1}{%
  ...
  %}%
    \boxcommand{grey}{\strut\textcolor{black}{\textsc{#2}}}
}% End of \NewPerson
\makeatother